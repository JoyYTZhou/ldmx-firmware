...
#ifndef __LDMX_DATA_SERVER_SVTBATCHHEADER_HH__
#define __LDMX_DATA_SERVER_SVTBATCHHEADER_HH__

/* ---------------------------------------------------------------------- *//*!

  \file   ldmx/data/server/SvtBatchHeader.hh
  \brief  Defines filling the SVT batch header structure.
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Fill the SVT Batch Header
                                                                          */
/* ---------------------------------------------------------------------- */
...
public:
   ...
   SvtBatchHeader (uint32_t sequence);

   SvtBatchHeader (uint32_t  version, 
                   uint32_t    width,
                   uint32_t sequence,
                   uint32_t     rsvd);
...
namespace server     {
/* ---------------------------------------------------------------------- */
inline SvtBatchHeader::SvtBatchHeader () :
       SvtBatchHeader (Version, Width, 0, Rsvd)
{
   return;
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
inline SvtBatchHeader::SvtBatchHeader (uint32_t sequence) :
       SvtBatchHeader (Version, Width, sequence, Rsvd)
{
...
/* ---------------------------------------------------------------------- */
inline SvtBatchHeader::SvtBatchHeader (uint32_t  version, 
                                       uint32_t    width,
                                       uint32_t sequence,
                                       uint32_t     rsvd) 
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] nbytes  The number of bytes to allocate
                                                                          */
/* ---------------------------------------------------------------------- */
inline void *SvtBatchHeader::operator
             new (size_t                       nbytes, 
                  ldmx::data::server::SvtBatch *batch)
{
...
/* --------------------------------------------------------------------- */
template<typename ValueType>
inline uint32_t SvtBatchHeader::add
                 (ValueType value, 
                  ldmx::data::common::SvtBatchHeader::Mask     mask,
                  ldmx::data::common::SvtBatchHeader::Offset offset)
{
...
/* --------------------------------------------------------------------- *//*!
  ...
  \return    The correctly masked and justified field
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addVersion ()
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] version The version number
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addVersion (uint32_t version)
{
   uint32_t field = add<uint8_t>(Version, Mask::Version, Offset::Version);
   return   field;
}
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \return    The correctly masked and justified field
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addWidth ()
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \note
   Since this field is set to AxiWidth64 for the SVTvalue, this 
   method is primarily provided for gobbering it so that any downstream
   verifier can be tested.
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addWidth (uint32_t width)
{
   uint32_t field = add<uint8_t>(        width,
                                 Mask  ::Width, 
                                 Offset::Width);
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] sequenceNumber The value of the sequence number
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addSequence (uint32_t sequenceNumber)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \return    The correctly masked and justified field
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addRsvd ()
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \note
   ...
   verifier can be tested.
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchHeader::addRsvd (uint32_t rsvd)
{
   uint32_t field = add<uint16_t>(rsvd, Mask::Rsvd, Offset::Rsvd);
   return   field;
}
