...
/* ---------------------------------------------------------------------- *//*!
  ...
  \file   ldmx/builder/server/TriggerReceiver.cc
...
/* LOCAL PROTOTYPES                                                       */
/* ---------------------------------------------------------------------- */
static Trigger *allocate (ldmx::utl::FixedPacket                     *fpa);

static int     readBatch (int                                          fd,
                          ldmx::data::client::TriggerBatch         *batch,
                          size_t                                  maxSize);

inline static uint64_t 
        getTriggerNumber (ldmx::data::client::TriggerEventLong const *evt,
                          bool                                     isLong);
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] path The path of the trigger pipe
                                                                          */
/* ---------------------------------------------------------------------- */
static inline int openPipe (char const *path)
{
   ...
   {
      fprintf (stderr,
               "Error:TriggerReceiver unable to open the trigger pipe <%s>\n",
               path);
      exit (-1);
   }
                  ...
                  int   packet_offset)
{
   ...
   ctb->m_type = Contribution::Type::Trigger;
   return;
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- *//*!
 ...
 \param[in] nevents  The maximum number of events in a trigger batch

                                                                          */
/* ---------------------------------------------------------------------- */
static inline int getBatchSize (int nevents)
{
   int    nbytes = ldmx::data::client::TriggerBatch::maxSize (nevents);
   return nbytes;
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Constructor for the trigger receiver

  \param[in] cfg  Configuration parameters
                                                                          */
/* ---------------------------------------------------------------------- */
TriggerReceiver::TriggerReceiver (ldmx::builder::client::Configuration const &cfg,
                                  Contributions                             *ctbs,
                                  int                                      ctbIdx) :
   ...
   m_fd            (openPipe (cfg.m_triggerPipe.c_str())),
   m_nbatches      (0),
   m_nevents       (0)
{
   return;
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Starts the trigger receiver thread
                                                                          */
/* ---------------------------------------------------------------------- */
void TriggerReceiver::start ()
{
   printf ("Starting trigger receiver\n");
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Receives trigger events

  \param[in] receiver The trigger receiver class
                                                                          */
/* ---------------------------------------------------------------------- */
void TriggerReceiver::receiver (TriggerReceiver *receiver)
{
   ...
   ldmx::data::client::TriggerHeader hdr;
   ldmx::utl::List               lclList;
   ldmx::data::client::TriggerBatch *batch = receiver->triggerBatch ();
   ...
   while (1)
   {
      ...
      // Read the trigger batch
      int nread __attribute__ ((unused));


      nread = readBatch (fd, batch, batchSize);
      ldmx::data::client::TriggerHeader *hdr = batch->getHeader ();
      ...
      // Retrieve the information needed to read the trigger events
      int      nevents = hdr->getBlockSize0       ();
      bool      isLong = hdr->getTimestampPresent ();
      size_t eventSize = isLong
                       ? sizeof (ldmx::data::client::TriggerEventLong) 
                       : sizeof (ldmx::data::client::TriggerEvent);

      printf ("Reading %d events\n", nevents);

      

      // Read in the number of events in this trigger batch
      for (int idx = 0; idx < nevents; idx++)
      {
         // Allocate enough storage for the long form of the trigger event
         auto *trg = allocate (fpa);


         // Locate the event
         ldmx::data::client::TriggerEventLong const
                            *evt = batch->getEventLong (idx);


         // Get the receive time and the event sequence number
         auto      rcvTime = ldmx::utl::Timestamp::current ();
         uint64_t sequence = getTriggerNumber (evt, isLong);
         ...
         printf ("TriggerEvent: nevents:%4d/%4d long:%d seq:%12.12lx nbyte:%zd\n", 
                 idx, nevents, isLong, sequence, eventSize);


         ::new (trg) Trigger (eventSize,
                              sequence, 
                              rcvTime,
                              evt,
                              last);
         ...
         // Place this trigger event on the local list
         lclList.insert (trg);
      }


      // Transfer the local list of trigger events to the communications list
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief   Allocates one trigger event from the specified fixed packet pool
  \return  The allocated trigger event
  ...
  \note
   ...
   and long form of the trigger event
                                                                          */
/* ---------------------------------------------------------------------- */
inline static Trigger *allocate (ldmx::utl::FixedPacket *fpa)
{
   Trigger *trg = reinterpret_cast<decltype(trg)>(fpa->getW ());
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief  Retrieve either the 32 or 48-bit trigger sequence number
  \return Either the 32 or 48-bit trigger sequence number

  \param[in]    evt The trigger event
                         ...
                         bool                                     isLong)
{
   uint64_t sequence = isLong 
                      ? evt->getTriggerNumber48 ()
                      : reinterpret_cast<ldmx::data::client::TriggerEvent const *>
                       (evt)->getTriggerNumber ();
   return sequence;
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Reads in the Trigger header

  \param[ in] fd  The file descriptor of the named pipe
  \param[out] hdr The memory to hold the trigger header
                                                                          */
/* ---------------------------------------------------------------------- */
inline static int readHeader (int fd, ldmx::data::client::TriggerHeader *hdr)
{
   ...
   {
      fprintf (stderr,
               "TriggerReceiver::receiver Error reading in header\n"
               "                          Needed:Got %zd:%zd\n",
               sizeof (*hdr), nread);
      exit (-1);
   }
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Reads in everything but trigger header for a trigger batch

  \param[ in]    fd  The file descriptor of the named pipe
  \param[out]   buf  The memory to hold the remainder of the trigger batch
...
inline static int readRest (int fd, void *buf, size_t nbytes)
{
   ...
   {
      fprintf (stderr,
               "TriggerReceiver::receiver Error reading in remainder\n"
               "                          Needed:Got %zd:%zd\n",
               nbytes, nread);
      exit (-1);
   }
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief  Read a batch of trigger events
  \return The number of events in the batch

  \param[ in]      fd The file descriptor to read from
  \param[out]   batch Pointer to the receiving trigger batch
                      ...
                      size_t                          maxSize)
{
   ...
   // Retrieve the information needed to read the rest of the trigger batch
   int      nevents = hdr->getBlockSize0       ();
   bool      isLong = hdr->getTimestampPresent ();
   size_t eventSize = isLong
                    ? sizeof (ldmx::data::client::TriggerEventLong) 
   ...
   size_t nbytes = nevents * eventSize + sizeof (ldmx::data::client::TriggerTrailer);
       

   // Read the remainder of the trigger batch
   printf ("Reading %d events\n", nevents);
