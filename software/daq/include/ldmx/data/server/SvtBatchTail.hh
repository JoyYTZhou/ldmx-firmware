...
#ifndef __LDMX_DATA_SERVER_SVTBATCHTAIL_HH__
#define __LDMX_DATA_SERVER_SVTBATCHTAIL_HH__

/* ---------------------------------------------------------------------- *//*!

  \file   ldmx/data/server/SvtBatchTail.hh
  \brief  Defines filling the SVT batch tail word structure.
  \author JJRussell - russell@slac.stanford.edu

  \par
   This 64-bit value acts as a separator between each SvtEvent.
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Fill the SVT Batch Tail
                                                                          */
/* ---------------------------------------------------------------------- */
class SvtBatchTail : public ldmx::data::common::SvtBatchTail
{
public:
   SvtBatchTail ();
   SvtBatchTail (uint32_t     nvalid,
                 bool      errorFlag);
   ...
   void *operator new (std::size_t                  nbytes, 
                       ldmx::data::server::SvtBatch *batch);
...
private:
   ...
   static uint32_t add (ValueType value, 
                        ldmx::data::common::SvtBatchTail::Mask     mask,
                        ldmx::data::common::SvtBatchTail::Offset offset);
};
/* ---------------------------------------------------------------------- */
} /* namespace server                                                     */
} /* namespace data                                                       */
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Default constructor
                                                                          */
/* ---------------------------------------------------------------------- */
inline SvtBatchTail::SvtBatchTail ()
{
   return;
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief  The simplified SVT Batch Cconstructor
  ...
  \par
   ...
   ignored by downstream code.
                                                                          */
/* ---------------------------------------------------------------------- */
inline SvtBatchTail::SvtBatchTail (uint32_t nvalid, 
                                   bool  errorFlag)
{
   m_nvalid = nvalid;
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Complete, \e i.e. no defaults, SVT batch tail constructor
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief  Custom SVT Batch Tail word Header allocator
  \return Pointer to the allocated class
                                                                          */
/* ---------------------------------------------------------------------- */
inline void *SvtBatchTail::
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \brief Convenience method to customize the general template method
         to add a field to be specific for the SVT Batch Header word.
...
/* ---------------------------------------------------------------------- */
template  <typename ValueType>
inline uint32_t SvtBatchTail::add 
                    (ValueType value, 
                     ldmx::data::common::SvtBatchTail::Mask     mask,
                     ldmx::data::common::SvtBatchTail::Offset offset)
{
   uint64_t field = ldmx::utl::bf::
            add<uint32_t, 
                ValueType, 
                ldmx::data::common::SvtBatchTail::Mask, 
                ldmx::data::common::SvtBatchTail::Offset>(value, 
                                                          mask,
                                                          offset);
     return field;
   }
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] tDest  The 8-bit TDEST value
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addTDest (uint8_t tDest)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] tUserFirst  The 8-bit TUSER_FIRST value
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addTUserFirst (uint8_t tUserFirst)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] errorFlag  The error flag
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addErrorFlag (bool errorFlag)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] tUserLast  The 8-bit TUSER_LAST value
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addTUserLast (uint8_t tUserLast)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \param[in] width The AXI bit stream width, This should be a value
                   ...
                   ldmx::common::SvtBatchTail 

  \note
   Since the AXI bit stream width for the SVT is currently fixed at 
   64 bits (width = 2), this method is primarily provided for completeness
   and testing purposes.
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addWidth (uint8_t width)
{
...
/* ---------------------------------------------------------------------- *//*!
  ...
  \note
   Since this field is currently fixed to 0 for the SVT this method is
   primarily provided for completeneess and testing purposes.
                                                                          */
/* ---------------------------------------------------------------------- */
inline uint32_t SvtBatchTail::addRsvd (uint8_t rsvd)
{
