...
#include "objdef.h"
#include "testutils.h"
#include "ts_s30xl_threshold_trigger_hw.h"
    ...
    while (TestVec) {
        ...
        if (readin) {
            ...
            if (s == "b") {
                ...
                if (dataReady_in[0] == 1) { amIOn = true; }
                continue;
            }
            std::cout << s << std::endl;
            chID        = s.substr(0, s.find(","));
            s           = s.substr(s.find(",") + 1);
            int counter = 0;
            while (counter < 12) {
            ...
            if (s == "a") {
                ...
                if (!(counter1 - counter2 == 0)) {
                    ...
                    if (amIOn) { counterErr += 1; }
                }
                amIOn    = false;
                counter1 = 0;
                continue;
            }
            std::cout << "ONE LDMX-SW HIT AT " << (std::stoi(s.substr(0, s.find(","))))
                      << " LIKE SO: " << std::stoi(s.substr(s.find(",") + 1)) << std::endl;
            counter1 += 1;
        }
    }
endloop:
